  ...
  return (
    <View className="bg-primary flex-1 ">
      <ScrollView contentContainerStyle={{paddingBottom:80}}>
        ...
        <View className='flex-col items-start justify-center mt-5 px-5'>
          ...
          <View className='flex-row items-center gap-x-1 mt-2'>
            <Text className='text-white font-bold'>{movie ?. release_date}</Text>
            <Text className='text-white font-bold'>| {movie ?. runtime} min </Text>
          </View>
          ...
          <View className='flex flex-row justify-between w-1/2 '>
            <MovieInfo label="Budget" value={`$${movie ?. budget / 1_000_000}M` || 'N/A'} />
            <MovieInfo label="Revenue" value={`$${movie ?. revenue / 1_000_000}M` || 'N/A'} />
          </View>
          <MovieInfo label="Production Company" value={movie ?. production_companies?.map((company) => company?.name).join(' - ') || 'N/A'} />
      ...
      <TouchableOpacity className='absolute bottom-5 left-0 right-0 mx-5 bg-accent rounded-lg py-3.5 flex-row items-center justify-center z-50' onPress={router.back}>
        <Image
          source={icons.arrow}
          className='size-5 mr-1 mt-0.5 rotate-180 tintcolor="#ffffff"'
        />
